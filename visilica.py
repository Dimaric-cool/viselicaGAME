...
playsound('start.mp3', False)  # не работает только через ыгидшьу

from myDB import *

hideturtle()

slovechki = ['Кант', 'Хроника', 'Зал', 'Галера', 'Балл', 'Вес', 'Кафель', 'Знак', 'Фильтр', 'Башня', 'Кондитер', 'Омар',
             'Чан', 'Пламя', 'Банк', 'Тетерев', 'Муж', 'Камбала', 'Груз', 'Кино', 'Лаваш', 'Калач', 'Геолог', 'Бальзам',
             'Бревно', 'Жердь', 'Борец', 'Самовар', 'Карабин', 'Подлокотник', 'Барак', 'Мотор', 'Шарж', 'Сустав',
             'Амфитеатр', 'Скворечник', 'Подлодка', 'Затычка', 'Ресница', 'Спичка', 'Кабан', 'Муфта', 'Синоптик',
             'Характер', 'Мафиози', 'Фундамент', 'Бумажник', 'Библиофил', 'Дрожжи', 'Казино', 'Конечность', 'Пробор',
             'Дуст', 'Комбинация', 'Мешковина', 'Процессор', 'Крышка', 'Сфинкс', 'Пассатижи', 'Фунт', 'Кружево',
             'Агитатор', 'Формуляр', 'Прокол', 'Абзац', 'Караван', 'Леденец', 'Кашпо', 'Баркас', 'Кардан', 'Вращение',
             'Заливное', 'Метрдотель', 'Клавиатура', 'Радиатор', 'Сегмент', 'Обещание', 'Магнитофон', 'Кордебалет',
             'Заварушка']
    ...
    if isGame:
        if not isDrawing:
            for letter in lettersDict:
                if (lettersDict[letter][0] - 25 < x < lettersDict[letter][0] + 25 and lettersDict[letter][1] < y <
                    lettersDict[letter][1] + 50) and (letter not in listUsedLet):
                    # print(letter)
                    penup()
...
def chek_latter(bukva):
    ...
    if bukva in sicret_word and bukva not in listUsedLet:
        schetWin += 1
        playsound('verno.mp3', False)  # не работает только через ыгидшьу
        # print("Угадал букву")
        ...
        schetLose += 1
        playsound('neverno.mp3', False)  # не работает только через ыгидшьу
        listUsedLet.append(bukva)
        # print("Не угадал")
...
def chek_game(win, lose, word):
    if lose == 10:
        menu("ВЫ ПРОИГРАЛИ")
        playsound('lo.mp3', False)
    elif win == len(set(word)):
        menu("ВЫ ВЫИГРАЛИ")
        playsound('wo1.mp3', False)
...
def start_game():
    ...
    goto(-450, 400)
    playerInfo['Games'] += 1
    write(f"Игрок: {player}\tВсего игр: {playerInfo['Games']}\tВсего очков: {playerInfo['Total Scores']}", move=False,
...
def table():
    ...
    for x in range(len(massiv)):
        ...
        goto(-300, y)
        write(f"{massiv[x][0]:<5}\t{massiv[x][2]:>15}\t{massiv[x][1]:>15}", move=False, align="left",
